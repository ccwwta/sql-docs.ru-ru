---
title: Формирование веб-каналов данных из отчетов (построитель отчетов) | Документация Майкрософт
description: Модуль подготовки отчетов служб Reporting Services Atom формирует сервисный документ, в котором перечислены веб-каналы в отчете с разбиением на страницы, а также веб-каналы из областей данных в нем.
ms.date: 05/30/2017
...
### <a name="working-with-data-feeds"></a>Работа с потоками данных  
 ...
 Для упрощения работы с веб-каналом данных можно воспользоваться свойством DataElementName области данных для выбора понятных имен. Если свойству DataElementName присвоено значение, его же будет использовать и подэлемент веб-канала данных \<**d**> вместо имени области данных по умолчанию. Например, если по умолчанию область данных имеет имя Tablix1, а свойству DataElementName присвоено значение SalesByTerritoryYear, то \<**d**> веб-канала данных использует значение SalesByTerritoryYear. Если области данных содержат два потока данных (как в случае с матричным отчетом, описанным выше), потоки данных будут иметь имена SalesByTerritoryYear _Territory и SalesByTerritoryYear _Year.  
