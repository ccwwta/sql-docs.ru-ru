---
description: Определения ролей — стандартные роли
...
##  <a name="content-manager-role"></a><a name="bkmk_content"></a> Роль "Диспетчер содержимого"  
...
|Управление отдельными подписками|Создание, просмотр и удаление принадлежащих пользователям подписок на отчеты и связанные отчеты. Эта задача также поддерживает изменение и выполнение [запланированного обновления для Power BI (PBIX-файлов) на сервере отчетов Power BI](/power-bi/report-server/configure-scheduled-refresh).|  
...
|Установка безопасности для отдельных элементов|Определение политики безопасности для отчетов, связанных отчетов, папок, ресурсов и источников данных. Дополнительные сведения см. в разделе [Защищаемые элементы](../../reporting-services/security/securable-items.md).|  
...
### <a name="customizing-the-my-reports-role"></a>Настройка роли "Мои отчеты"  
 ...
 Хотя задача "Установка безопасности для отдельных элементов" не является частью определения роли по умолчанию, можно добавить эту задачу к роли **Мои отчеты**, чтобы пользователи могли изменять параметры безопасности для вложенных папок и отчетов.  
