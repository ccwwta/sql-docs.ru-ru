---
description: Создание роли RSExecRole
...
### <a name="to-create-rsexecrole-in-the-master-system-database-using-management-studio"></a>Создание роли RSExecRole в системной базе данных master с помощью Management Studio  
...
7.  Щелкните правой кнопкой мыши **Роли базы данных** и выберите пункт **Создать роль базы данных**. Откроется **страница создания роли базы данных**.  
...
9.  Нажмите кнопку **Поиск**. Отображается диалоговое окно **Добавление объектов** . Параметр **Выбрать объекты** выбран по умолчанию.  
...
16. Нажмите кнопку **ОК**, а затем снова щелкните **ОК**.  
  
17. Выберите первую хранимую процедуру sp_add_category.  
  
18. В строке **Выполнить** в столбце **Предоставить** установите флажок.  
...
20. На той же странице **Защищаемые объекты** еще раз щелкните **Поиск**. Отображается диалоговое окно **Добавление объектов** . Параметр **Выбрать объекты** выбран по умолчанию.  
...
29. В строке **Выбрать** в столбце **Предоставить** установите флажок.  
...
## <a name="move-the-report-server-database"></a>Перемещение базы данных сервера отчетов  
 После создания ролей можно переместить базу данных сервера отчетов на новый экземпляр SQL Server. Дополнительные сведения см. в разделе [Перемещение баз данных сервера отчетов на другой компьютер](../../reporting-services/report-server/moving-the-report-server-databases-to-another-computer-ssrs-native-mode.md).  
  
 Если [!INCLUDE[ssDE](../../includes/ssde-md.md)] обновляется до SQL Server 2016, эту процедуру можно выполнить до или после перемещения базы данных.  
  
 База данных сервера отчетов обновляется автоматически при подключении к ней сервера отчетов. Для обновления этой базы данных не требуется никаких конкретных шагов.  
...
5.  Введите имя экземпляра сервера компонента Database Engine. Если базы данных сервера отчетов подключены к именованному экземпляру, то имя экземпляра необходимо вводить в следующем формате: \<servername>\\<instancename\>.  
  
6.  Нажмите кнопку **Проверить соединение**. Должно появиться диалоговое окно с сообщением "Проверка подключения выполнена успешно".
...
14. Щелкните **URL-адрес веб-портала**.  
  
15. Щелкните ссылку, чтобы открыть веб-портал. Будут отображены элементы сервера отчетов из базы данных сервера отчетов.  

## <a name="creating-the-rsexecrole-role-and-permissions-using-t-sql"></a>Создание разрешений и роли RSExecRole с помощью T-SQL
Создать роль и присвоить необходимые разрешения в системных базах данных можно также с помощью следующего скрипта T-SQL:
```sql
USE master;
GO
