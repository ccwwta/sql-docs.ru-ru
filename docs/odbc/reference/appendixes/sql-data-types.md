...
|SQL_DECIMAL|ДЕСЯТИЧное число (*p*,*s*)|Точное числовое значение со знаком, имеющее точность не менее *p* и Scale *s.* (Максимальная точность определяется драйвером.) (1 <= *p* <= 15; *с*  <=  *p*). четырех|  
|SQL_NUMERIC|ЧИСЛовые (*p*,*s*)|Точное числовое значение со знаком с точностью *p* и масштабом *s* (1 <= *p* <= 15; *с*  <=  *p*). четырех|  
...
|SQL_INTEGER|ЦЕЛОЕ ЧИСЛО|Точное числовое значение с точностью 10 и масштабом 0 (со знаком:-2 [31] <= *n* <= 2 [31]-1, без знака: 0 <= *n* <= 2 [32]-1) [3].|  
|SQL_REAL|ВЕЩЕСТВЕННОЕ ЧИСЛО|Приблизительное числовое значение со знаком с двоичной точностью 24 (ноль или абсолютное значение 10 [-38] — 10 [38]).|  
