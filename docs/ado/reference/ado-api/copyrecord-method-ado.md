 ...
 *Источник*  
 Необязательный параметр. **Строковое** значение, содержащее URL-адрес, указывающий копируемую сущность (например, файл или каталог). Если параметр *Source* не указан или указывает пустую строку, будет скопирован файл или каталог, представленный текущей [записью](./record-object-ado.md) .  
 ...
 *Параметры*  
 Необязательный параметр. Значение [копирекордоптионсенум](./copyrecordoptionsenum.md) , имеющее значение по умолчанию **адкопюнспеЦифиед**. Задает поведение этого метода.  
