...
## <a name="permissions"></a>Разрешения  
 Требуется разрешение `VIEW SERVER STATE`. Кроме того, требуются отдельные разрешения на каждую структуру плана, создаваемую с помощью хранимой процедуры sp_create_plan_guide_from_handle. Для создания структуры плана типа OBJECT требуется `ALTER` разрешение на упоминаемый объект. Для создания структуры плана типа SQL или TEMPLATE требуется `ALTER` разрешение в текущей базе данных. Чтобы определить тип создаваемой структуры плана, выполните следующий запрос.  
