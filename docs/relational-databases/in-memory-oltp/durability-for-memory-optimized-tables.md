...
### <a name="the-delta-file"></a>Разностный файл  
 ...
 Размер. Каждый разностный файл имеет размер примерно в 16 МБ для компьютеров с памятью больше 16 ГБ и 1 МБ для компьютеров с памятью, меньшей или равной 16 ГБ. Начиная с версии [!INCLUDE[sssql16-md](../../includes/sssql16-md.md)] SQL Server может использоваться режим больших контрольных точек, если определено, что подсистема хранения обладает достаточным быстродействием. В режиме больших контрольных точек разностные файлы имеют размер 128 МБ.  
